...
def server_handler(bound_ip, bound_port, buffer_size, timeout_seconds, max_clients, max_clients_per_ip, server_delay, auto_save_delay, libs):	# Server handler thread (sets up the server and handles logs).
	try:
		...
		thread = multiprocessing.Process(target=network_core.server_thread, args=(server, q, buffer_size, timeout_seconds, max_clients, max_clients_per_ip, server_delay, queue_utils, cmd, libs))	# Set up a child process for the server thread.
...
def main():
	...
	server_handler(settings['bound_ip'], settings['bound_port'], settings['buffer_size'], settings['timeout_seconds'], settings['max_clients'], settings['max_clients_per_ip'], settings['server_delay'], settings['auto_save_delay'], libs)
